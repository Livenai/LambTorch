...
def print_best_options(list_of_tasks):
    """ Imprime la mejor opcion de la lista de tareas. """
    best_loss = -1
    best = None
    best_index = None
    ...
    for index in range(len(list_of_tasks)):
        ...
        tloss, dloss, fw, net = selected['test_loss'], selected['dev_loss'],
                                selected['fw'], selected['gnn_network']
        # print (loss, fw, net)
...
def get_random_hyperparameters(identifier):
    """ Genera hiperparametros aleatorios para adaptados a una red GNN """
    ...
    gnn_layers = randrange(start=3, stop=6)
